...
##############################
from projects.llava_sam2.models.temporal_grounding_model import (
    # SpatialTemporalPositionalEncoding, 
...
class Sa2VAChatModel(PreTrainedModel):
    ...
    def __init__(self, config: Sa2VAChatConfig, vision_model=None, language_model=None, use_flash_attn=True):
        ...
        # self.ca_layers = CrossAttentionStack(num_layers=3, embed_dim=embed_dim, ff_dim=embed_dim*2)
        ...
        # #########################################
        # # temporal grounding专属 (query-based)
        # self.num_query_tokens = 128
        ...
        # self.temporal_encoding = PooledTemporalPositionalEncoding(
        #     hidden_dim=1024,
        ...
        #########################################
        # # temporal grounding专属 (pool-direct-based)
        self.query_hidden_dim = 1024
        self.max_frames = 64
        # self.temporal_encoding = PooledTemporalPositionalEncoding(
        #     hidden_dim=1024,
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        dense_frames_per_batch = data.get('dense_frames_per_batch', None)
        sparse_frames_per_batch = data.get('sparse_frames_per_batch', None)
        use_dense_sparse = data.get('use_dense_sparse', False)
        ...
        outputs = self._llm_forward_compress(
            ...
            vp_overall_mask=vp_overall_mask,
            prompt_masks=prompt_masks,
            dense_frames_per_batch=dense_frames_per_batch,
            sparse_frames_per_batch=sparse_frames_per_batch,
            use_dense_sparse=use_dense_sparse,
        )
    ...
    def _llm_forward_compress(
        ...
        # 分别处理dense和sparse token
        dense_selected = (input_ids == self.img_context_token_id)
        sparse_selected = (input_ids == getattr(self, 'sparse_img_context_token_id', -1))  # 如果没有定义则为-1，不会匹配任何token

        if vp_embeds is None:
            try:
                # 处理dense token (完整的vision features)
                if dense_selected.sum() > 0:
                    dense_count = dense_selected.sum()
                    dense_vit_embeds = vit_embeds[:dense_count]  # 取前面的dense特征
                    input_embeds[dense_selected] = dense_vit_embeds.reshape(-1, C)
                
                # 处理sparse token (pooled features)  
                if sparse_selected.sum() > 0:
                    sparse_count = sparse_selected.sum()
                    sparse_vit_embeds = vit_embeds[dense_selected.sum():dense_selected.sum() + sparse_count]  # 取sparse特征
                    input_embeds[sparse_selected] = sparse_vit_embeds.reshape(-1, C)
                    
            except Exception as e:
                # 兼容原有逻辑，如果出错则使用原来的方式
                selected = dense_selected | sparse_selected
                vit_embeds = vit_embeds.reshape(-1, C)
                print(f'warning: {e}, input_embeds[selected].shape='
                    f'{input_embeds[selected].shape}, '
                    f'vit_embeds.shape={vit_embeds.shape}')
            ...
            try:
                ...
                if dense_selected.sum() > 0:
                    dense_count = dense_selected.sum()
                    dense_vp_embeds = vp_embeds[:dense_count]  # 取前面的dense特征
                    input_embeds[dense_selected] = dense_vp_embeds.reshape(-1, C)
                
                # 处理sparse token (pooled features)  
                if sparse_selected.sum() > 0:
                    sparse_count = sparse_selected.sum()
                    sparse_vp_embeds = vp_embeds[dense_selected.sum():dense_selected.sum() + sparse_count]  # 取sparse特征
                    input_embeds[sparse_selected] = sparse_vp_embeds.reshape(-1, C)
